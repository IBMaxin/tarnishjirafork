...
class CloseInterfacePacketListenerTest {
    private Player player;
    private CloseInterfacePacketListener listener;

    @BeforeEach
    void setUp() {
        player = new Player("TestPlayer");
        listener = new CloseInterfacePacketListener();
    }

    @Test
    void handlePacket_closeWindowOpcode_closesInterface() {
        player.interfaceManager.setMain(100);
        player.interfaceManager.setDialogue(200);
        GamePacket packet = createPacket(ClientPackets.CLOSE_WINDOW);
        listener.handlePacket(player, packet);
        assertEquals(-1, player.interfaceManager.getMain());
        assertTrue(player.interfaceManager.isMainClear());
        assertTrue(player.interfaceManager.isDialogueClear());
    }

    @Test
    void handlePacket_wrongOpcode_doesNotCloseInterface() {
        player.interfaceManager.setMain(100);
        GamePacket packet = createPacket(0);
        listener.handlePacket(player, packet);
        assertEquals(100, player.interfaceManager.getMain());
        assertFalse(player.interfaceManager.isMainClear());
    }
    ...
    void handlePacket_emptyPayload_doesNotThrow() {
        ...
        GamePacket packet = new GamePacket(ClientPackets.CLOSE_WINDOW, PacketType.FIXED, buffer);
