...
class DialoguePacketListenerTest {

    private Player player;
    private DialoguePacketListener listener;

    @BeforeEach
    void setUp() {
        player = new Player("TestPlayer");
        listener = new DialoguePacketListener();
    }

    @Test
    void handlePacket_validPacket_queuesDialogueEventAndProcessesWithoutError() {
        GamePacket packet = createDialoguePacket();
    ...
    @Test
    void handlePacket_eventConsumedAfterFirstProcess_secondProcessDoesNothing() {
        GamePacket packet = createDialoguePacket();
    ...
    private static GamePacket createDialoguePacket() {
        ByteBuf buffer = Unpooled.buffer(0);
        return new GamePacket(40, PacketType.FIXED, buffer);
    }
}