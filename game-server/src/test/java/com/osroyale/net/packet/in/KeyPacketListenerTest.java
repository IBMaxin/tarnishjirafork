...
class KeyPacketListenerTest {

    private Player player;
    private KeyPacketListener listener;

    @BeforeEach
    void setUp() throws Exception {
        player = new Player("TestPlayer");
        listener = new KeyPacketListener();
    }

    @Test
    void handlePacket_validKey_queuesEventAndProcessesWithoutThrowing() {
        GamePacket packet = createPacket(186, (byte) 0, (byte) 1);
    ...
    @Test
    void handlePacket_negativeKey_doesNotQueueEvent() {
        GamePacket packet = createPacket(186, (byte) -1, (byte) -1);
    ...
    @Test
    void handlePacket_locked_doesNotQueueEvent() throws Exception {
        Locking locking = mock(Locking.class);
        when(locking.locked(PacketType.KEY)).thenReturn(true);
    ...
    void handlePacket_malformedPacketTooShort_throwsIndexOutOfBounds() {
        ...
        buffer.writeByte(0);
        GamePacket packet = new GamePacket(186, com.osroyale.net.packet.PacketType.FIXED, buffer);
    ...
    private static GamePacket createPacket(int opcode, byte... payload) {
        ...
        return new GamePacket(opcode, com.osroyale.net.packet.PacketType.FIXED, buffer);
    }
}